...
KernelState::KernelState(memory::Memory* memory, runtime::FunctionDispatcher* function_dispatcher,
...
IAudioSystem* KernelState::audio_system() const {
  return audio_system_;
}

audio::AudioSystem* KernelState::native_audio_system() const {
  return dynamic_cast<audio::AudioSystem*>(audio_system());
}

audio::XmaDecoder* KernelState::native_xma_decoder() const {
  auto* audio_system = native_audio_system();
  return audio_system ? audio_system->xma_decoder() : nullptr;
}

runtime::ExportResolver* KernelState::export_resolver() const {
  return export_resolver_;
}

IGraphicsSystem* KernelState::graphics_system() const {
  return graphics_system_;
}

input::InputSystem* KernelState::input_system() const {
  return input_system_;
}

ui::WindowedAppContext* KernelState::app_context() const {
  return app_context_;
}

ui::ImGuiDrawer* KernelState::imgui_drawer() const {
...
object_ref<XThread> KernelState::LaunchModule(object_ref<UserModule> module) {
  ...
  // TODO(tomc): if guest launch ever needs prelaunch dispatcher work, route it
  // through function_dispatcher_ directly rather than a broad runtime object.
...
void KernelState::SetExecutableModule(object_ref<UserModule> module) {
  ...
  auto* resolver = export_resolver();
  auto export_entry =
      resolver ? resolver->GetExportByOrdinal("xboxkrnl.exe", 0x0193 /* XexExecutableModuleHandle */)
               : nullptr;
  if (export_entry) {
  ...
  export_entry =
      resolver ? resolver->GetExportByOrdinal("xboxkrnl.exe", 0x01AF /* ExLoadedImageName */)
               : nullptr;
  if (export_entry) {
...
void KernelState::CompleteOverlappedEx(uint32_t overlapped_ptr, X_RESULT result,
  ...
  if (event_handle) {
    ...
    if (!ev) {
      REXSYS_WARN("CompleteOverlappedEx: missing XEvent for handle {:08X}",
