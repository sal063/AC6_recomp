...
XboxkrnlModule::XboxkrnlModule(KernelState* kernel_state)
  ...
  if (!REXCVAR_GET(kernel_debug_monitor)) {
    pKeDebugMonitorData = memory()->SystemHeapAlloc(4);
    auto lpKeDebugMonitorData = memory()->TranslateVirtual(pKeDebugMonitorData);
    memory::store_and_swap<uint32_t>(lpKeDebugMonitorData, 0);
  } else {
    pKeDebugMonitorData = memory()->SystemHeapAlloc(4 + sizeof(X_KEDEBUGMONITORDATA));
    memory::store_and_swap<uint32_t>(memory()->TranslateVirtual(pKeDebugMonitorData),
                                     pKeDebugMonitorData + 4);
    auto lpKeDebugMonitorData =
        memory()->TranslateVirtual<X_KEDEBUGMONITORDATA*>(pKeDebugMonitorData + 4);
  ...
  if (!REXCVAR_GET(kernel_cert_monitor)) {
    pKeCertMonitorData = memory()->SystemHeapAlloc(4);
    auto lpKeCertMonitorData = memory()->TranslateVirtual(pKeCertMonitorData);
    memory::store_and_swap<uint32_t>(lpKeCertMonitorData, 0);
  } else {
    pKeCertMonitorData = memory()->SystemHeapAlloc(4 + sizeof(X_KECERTMONITORDATA));
    memory::store_and_swap<uint32_t>(memory()->TranslateVirtual(pKeCertMonitorData),
                                     pKeCertMonitorData + 4);
    auto lpKeCertMonitorData =
        memory()->TranslateVirtual<X_KECERTMONITORDATA*>(pKeCertMonitorData + 4);
  ...
  uint32_t pXboxHardwareInfo = memory()->SystemHeapAlloc(16);
  auto lpXboxHardwareInfo = memory()->TranslateVirtual(pXboxHardwareInfo);
  ...
  uint32_t pExConsoleGameRegion = memory()->SystemHeapAlloc(4);
  auto lpExConsoleGameRegion = memory()->TranslateVirtual(pExConsoleGameRegion);
  ...
  uint32_t ppXexExecutableModuleHandle = memory()->SystemHeapAlloc(4);
  ...
  // variable.
  uint32_t ppExLoadedImageName = memory()->SystemHeapAlloc(kExLoadedImageNameSize);
  ...
  uint32_t pExLoadedCommandLine = memory()->SystemHeapAlloc(command_line_length);
  auto lpExLoadedCommandLine = memory()->TranslateVirtual(pExLoadedCommandLine);
  ...
  uint32_t pXboxKrnlVersion = memory()->SystemHeapAlloc(8);
  auto lpXboxKrnlVersion = memory()->TranslateVirtual(pXboxKrnlVersion);
  ...
  uint32_t pKeTimeStampBundle = memory()->SystemHeapAlloc(24);
  auto lpKeTimeStampBundle = memory()->TranslateVirtual(pKeTimeStampBundle);
