// Native runtime - Windows memory-mapped file I/O
  ...
  class Chunk {
   public:
    ...
    bool Open(const std::filesystem::path& path, bool low_address_space) {
      ...
      // If specified, try to place the mapping in the lower 32-bit address
      // space. This is used by the JIT code cache.
      if (low_address_space) {
        ...
        for (int i = 0; i < 1000; ++i) {
          ...
          data_ += capacity_;
          if (!successful) {
            REXLOG_ERROR("Unable to find space for mapping");
            data_ = nullptr;
            return false;
          }
        }
