...
bool ShouldAssertOnPacketExecutionFailure(const std::atomic<bool>& worker_running) {
  return worker_running.load(std::memory_order_acquire);
}
...
uint32_t CommandProcessor::ExecutePrimaryBuffer(uint32_t read_index, uint32_t write_index) {
  ...
  do {
    if (!ExecutePacket(&reader)) {
      ...
      if (ShouldAssertOnPacketExecutionFailure(worker_running_)) {
        assert_always();
      }
      REXGPU_WARN("Ignoring primary ringbuffer packet failure during shutdown.");
      break;
    }
...
void CommandProcessor::ExecuteIndirectBuffer(uint32_t ptr, uint32_t count) {
  ...
  do {
    if (!ExecutePacket(&reader)) {
      ...
      REXGPU_WARN("Ignoring indirect ringbuffer packet failure during shutdown.");
      break;
    }
  } while (reader.read_count());
...
void CommandProcessor::ExecutePacket(uint32_t ptr, uint32_t count) {
  ...
  do {
    if (!ExecutePacket(&reader)) {
      ...
      REXGPU_WARN("Ignoring packet execution failure during shutdown.");
      break;
    }
  } while (reader.read_count());
}
