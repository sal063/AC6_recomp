...
void RtlInitUnicodeString_entry(ppc_ptr_t<::rex::system::X_UNICODE_STRING> destination,
                                ppc_pchar16_t source) {
  if (source) {
...
void RtlFreeUnicodeString_entry(ppc_ptr_t<::rex::system::X_UNICODE_STRING> string) {
  if (string->pointer) {
    REX_KERNEL_MEMORY()->SystemHeapFree(string->pointer);
  }
...
void RtlCopyUnicodeString_entry(ppc_ptr_t<::rex::system::X_UNICODE_STRING> destination,
                                ppc_ptr_t<::rex::system::X_UNICODE_STRING> source) {
  if (!source) {
    destination->length = 0;
    return;
  }
...
ppc_u32_result_t RtlUnicodeStringToAnsiString_entry(ppc_ptr_t<X_ANSI_STRING> destination_ptr,
                                                    ppc_ptr_t<::rex::system::X_UNICODE_STRING>
                                                        source_ptr,
