...
PPC_FUNC_IMPL(rex_sub_823AD9C0) {
    PPC_FUNC_PROLOGUE();
    const uint32_t singleton_ptr = ctx.r3.u32;
    g_last_movie_singleton_ptr.store(singleton_ptr, std::memory_order_relaxed);
    const uint32_t worker_count_hint =
        singleton_ptr != 0 ? PPC_LOAD_U32(singleton_ptr + 304) : 0;
    if (!REXCVAR_GET(ac6_movie_audio_force_sync_dispatch) || worker_count_hint != 0) {
        if (REXCVAR_GET(ac6_movie_audio_force_sync_dispatch) && worker_count_hint != 0 &&
            (REXCVAR_GET(ac6_movie_audio_trace) ||
             ac6::audio_policy::IsDeepTraceEnabled())) {
            if (ac6::audio_policy::IsDeepTraceEnabled()) {
                const uint64_t async_vfunc_trace_count =
                    g_movie_audio_async_vfunc_trace_count.fetch_add(
                        1, std::memory_order_relaxed) +
                    1;
                const bool should_trace_async_vfunc =
                    async_vfunc_trace_count <= 64 || (async_vfunc_trace_count % 180) == 0;
                if (should_trace_async_vfunc) {
                    const uint32_t vfunc_obj = LoadGuestWord(base, singleton_ptr, 64);
                    const uint32_t vtable_ptr = LoadGuestWord(base, vfunc_obj, 0);
                    const uint32_t vfunc_target = LoadGuestWord(base, vtable_ptr, 68);
                    REXAPU_DEBUG(
                        "AC6 movie worker async vfunc singleton={:08X} obj={:08X} "
                        "vtable={:08X} target={:08X}",
                        singleton_ptr,
                        vfunc_obj,
                        vtable_ptr,
                        vfunc_target);
            ...
            if (ac6::audio_policy::IsDeepTraceEnabled()) {
                const uint64_t dispatch_trace_count =
                    g_movie_audio_worker_dispatch_trace_count.fetch_add(
                        1, std::memory_order_relaxed) +
                    1;
                const bool should_trace_dispatch =
                    dispatch_trace_count <= 64 || (dispatch_trace_count % 180) == 0;
                if (should_trace_dispatch) {
                    const uint32_t singleton_state_292 =
                        singleton_ptr != 0
                            ? static_cast<uint32_t>(PPC_LOAD_U8(singleton_ptr + 292))
                            : 0;
                    ...
                    const uint32_t primary0_word_1 = LoadGuestWord(base, singleton_primary_0, 4);
                    const uint32_t primary0_word_2 = LoadGuestWord(base, singleton_primary_0, 8);
                    const uint32_t primary0_word_3 =
                        LoadGuestWord(base, singleton_primary_0, 12);
                    const uint32_t primary1_word_0 = LoadGuestWord(base, singleton_primary_1, 0);
                    const uint32_t primary1_word_1 = LoadGuestWord(base, singleton_primary_1, 4);
                    const uint32_t primary1_word_2 = LoadGuestWord(base, singleton_primary_1, 8);
                    const uint32_t primary1_word_3 =
                        LoadGuestWord(base, singleton_primary_1, 12);
                    REXAPU_DEBUG(
                        "AC6 cutscene audio keeping guest worker dispatch singleton={:08X} "
                        "worker_count={} sync_override_bypassed=true state_292={:02X} "
                        "primary=[{:08X} {:08X} {:08X}] extra=[{:08X} {:08X} {:08X}] "
                        "primary0_head=[{:08X} {:08X} {:08X} {:08X}] "
                        "primary1_head=[{:08X} {:08X} {:08X} {:08X}]",
                        singleton_ptr,
                ...
                REXAPU_DEBUG(
                    "AC6 cutscene audio keeping guest worker dispatch singleton={:08X} "
                    "worker_count={} sync_override_bypassed=true",
                    singleton_ptr,
                    worker_count_hint);
            }
        }
    ...
    if (REXCVAR_GET(ac6_movie_audio_trace) ||
        ac6::audio_policy::IsDeepTraceEnabled()) {
        REXAPU_DEBUG(
            "AC6 cutscene audio sync vfunc result singleton={:08X} result={:08X}",
            ctx.r31.u32, static_cast<uint32_t>(ctx.r3.u32));
    }
    ctx.cr6.compare<int32_t>(ctx.r3.s32, 0, ctx.xer);
    if (ctx.cr6.lt) {
        goto loc_823ADABC_sync;
...
PPC_FUNC_IMPL(rex_sub_823B7C80) {
    ...
    ctx.lr = 0x823B7CA0;
    rex_sub_823B9F10(ctx, base);

    const int32_t stage1_result = ctx.r3.s32;
    const uint32_t primary_0_after_stage1 = LoadGuestWord(base, singleton_ptr, 80);
    const uint32_t primary_1_after_stage1 = LoadGuestWord(base, singleton_ptr, 84);
    const uint32_t primary_2_after_stage1 = LoadGuestWord(base, singleton_ptr, 88);
...
PPC_FUNC_IMPL(rex_sub_823B3BA8) {
    ...
    if (ac6::audio_policy::IsMovieAudioActive()) {
        ...
        if (should_trace) {
            ...
            const uint32_t voice_count = movie_state_ptr != 0 ? PPC_LOAD_U8(movie_state_ptr + 56) : 0;
            const uint32_t state_292 = movie_state_ptr != 0 ? PPC_LOAD_U8(movie_state_ptr + 292) : 0;
