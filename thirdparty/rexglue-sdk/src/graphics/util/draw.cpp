...
constexpr uint32_t kRelaxedMemExportRangeSizeMax = 4 * 1024 * 1024;
constexpr uint32_t kMemExportValidationShiftX = 30;
constexpr uint32_t kMemExportValidationShiftYZW = 23;
constexpr uint32_t kMemExportValidationValueX = 0x1;
constexpr uint32_t kMemExportValidationValueYZW = 0x96;

enum class MemExportRangeMatchMode { kStrict, kRelaxed };

bool IsMemExportAddressRuntimeCompatible(
    const xenos::xe_gpu_memexport_stream_t& stream) {
  return (stream.dword_0 >> kMemExportValidationShiftX) == kMemExportValidationValueX &&
         (stream.dword_1 >> kMemExportValidationShiftYZW) == kMemExportValidationValueYZW &&
         (stream.dword_2 >> kMemExportValidationShiftYZW) == kMemExportValidationValueYZW &&
         (stream.dword_3 >> kMemExportValidationShiftYZW) == kMemExportValidationValueYZW;
}

bool TryGetMemExportRange(const FormatInfo& format_info,
                          const xenos::xe_gpu_memexport_stream_t& stream,
                          MemExportRangeMatchMode match_mode, uint32_t& base_address_dwords_out,
                          uint32_t& size_bytes_out) {
  if (format_info.type != FormatType::kResolvable || !stream.index_count) {
    return false;
  }

  uint64_t size_bytes = uint64_t(stream.index_count) * (format_info.bits_per_pixel >> 3);
  if (!size_bytes || size_bytes > SharedMemory::kBufferSize) {
    return false;
  }

  uint64_t start_bytes = uint64_t(stream.base_address) << 2;
  uint64_t end_bytes = start_bytes + size_bytes;
  if (end_bytes > SharedMemory::kBufferSize) {
    return false;
  }

  if (match_mode == MemExportRangeMatchMode::kStrict) {
    if (stream.const_0x1 != 0x1 || stream.const_0x4b0 != 0x4B0 || stream.const_0x96 != 0x96) {
      return false;
    }
  } else {
    if (size_bytes > kRelaxedMemExportRangeSizeMax ||
        !IsMemExportAddressRuntimeCompatible(stream)) {
      return false;
    }
  }

  base_address_dwords_out = stream.base_address;
  size_bytes_out = uint32_t(size_bytes);
  return true;
}

void AddOrMergeMemExportRange(uint32_t base_address_dwords, uint32_t size_bytes,
                              std::vector<MemExportRange>& ranges_out) {
  uint64_t merged_start = uint64_t(base_address_dwords) << 2;
  uint64_t merged_end = merged_start + size_bytes;
  for (size_t i = 0; i < ranges_out.size();) {
    uint64_t existing_start = uint64_t(ranges_out[i].base_address_dwords) << 2;
    uint64_t existing_end = existing_start + ranges_out[i].size_bytes;
    if (merged_start > existing_end || existing_start > merged_end) {
      ++i;
      continue;
    }
    merged_start = std::min(merged_start, existing_start);
    merged_end = std::max(merged_end, existing_end);
    ranges_out.erase(ranges_out.begin() + i);
  }
  ranges_out.emplace_back(uint32_t(merged_start >> 2), uint32_t(merged_end - merged_start));
}

bool AddMemExportRangeFromStream(const FormatInfo& format_info,
                                 const xenos::xe_gpu_memexport_stream_t& stream,
                                 std::vector<MemExportRange>& ranges_out,
                                 MemExportRangeMatchMode match_mode) {
  uint32_t stream_base_address_dwords, stream_size_bytes;
  if (!TryGetMemExportRange(format_info, stream, match_mode, stream_base_address_dwords,
                            stream_size_bytes)) {
    return false;
  }
  ...
  AddOrMergeMemExportRange(stream_base_address_dwords, stream_size_bytes, ranges_out);
  return true;
}
...
void AddMemExportRanges(const RegisterFile& regs, const Shader& shader,
  ...
  static bool logged_relaxed_scan = false;
  bool found_explicit_range = false;
  for (uint32_t constant_index : shader.memexport_stream_constants()) {
    uint32_t float_constant_index = float_constants_base + constant_index;
    if (float_constant_index >= 512) {
      continue;
    }
    xenos::xe_gpu_memexport_stream_t stream =
        regs.GetMemExportStream(float_constant_index);
    const FormatInfo& format_info = *FormatInfo::Get(xenos::TextureFormat(stream.format));
    found_explicit_range |= AddMemExportRangeFromStream(
        format_info, stream, ranges_out, MemExportRangeMatchMode::kStrict);
  }

  if (found_explicit_range) {
    return;
  }

  bool found_explicit_relaxed_range = false;
  for (uint32_t constant_index : shader.memexport_stream_constants()) {
    uint32_t float_constant_index = float_constants_base + constant_index;
    if (float_constant_index >= 512) {
      continue;
    ...
    xenos::xe_gpu_memexport_stream_t stream =
        regs.GetMemExportStream(float_constant_index);
    const FormatInfo& format_info = *FormatInfo::Get(xenos::TextureFormat(stream.format));
    found_explicit_relaxed_range |= AddMemExportRangeFromStream(
        format_info, stream, ranges_out, MemExportRangeMatchMode::kRelaxed);
  }
  if (found_explicit_relaxed_range) {
    if (!logged_relaxed_scan) {
      logged_relaxed_scan = true;
      REXGPU_WARN(
          "Memexport range gather fell back to relaxed stream detection with capped range size");
    }
    return;
  }

  const Shader::ConstantRegisterMap& constant_map = shader.constant_register_map();
  auto scan_constants = [&](MemExportRangeMatchMode match_mode,
                            bool scan_all_float_constants) -> bool {
    bool found = false;
    auto try_scan_constant = [&](uint32_t shader_constant_index) {
      if (shader_constant_index >= 256) {
        return;
      }
      uint32_t float_constant_index = float_constants_base + shader_constant_index;
      if (float_constant_index >= 512) {
        return;
      }
      xenos::xe_gpu_memexport_stream_t stream =
          regs.GetMemExportStream(float_constant_index);
      const FormatInfo& format_info = *FormatInfo::Get(xenos::TextureFormat(stream.format));
      found |= AddMemExportRangeFromStream(format_info, stream, ranges_out, match_mode);
    };
    if (scan_all_float_constants || constant_map.float_dynamic_addressing) {
      for (uint32_t constant_index = 0; constant_index < 256; ++constant_index) {
        try_scan_constant(constant_index);
      }
      return found;
    }
    for (uint32_t block_index = 0; block_index < rex::countof(constant_map.float_bitmap);
         ++block_index) {
      uint64_t bitmap = constant_map.float_bitmap[block_index];
      while (bitmap) {
        uint32_t bit_index;
        rex::bit_scan_forward(bitmap, &bit_index);
        bitmap &= ~(uint64_t(1) << bit_index);
        try_scan_constant(block_index * 64 + bit_index);
      }
    }
    return found;
  };

  bool found_conservative_range = scan_constants(MemExportRangeMatchMode::kStrict, false);

  static bool logged_conservative_scan = false;
  if (found_conservative_range && !logged_conservative_scan) {
    logged_conservative_scan = true;
    REXGPU_INFO(
        "Memexport range gather fell back to conservative float-constant scanning for a shader");
  }
  if (found_conservative_range) {
    return;
  }

  bool found_relaxed_range = scan_constants(MemExportRangeMatchMode::kRelaxed, true);
  if (found_relaxed_range && !logged_relaxed_scan) {
    logged_relaxed_scan = true;
    REXGPU_WARN(
        "Memexport range gather fell back to relaxed stream detection with capped range size");
  }
...
bool GetResolveInfo(const RegisterFile& regs, const memory::Memory& memory,
  ...
  int32_t x0_scissored = x0;
  int32_t y0_scissored = y0;
  int32_t x1_scissored = x1;
  int32_t y1_scissored = y1;
  ...
  if (x0 >= x1 || y0 >= y1) {
    REXGPU_WARN(
        ...
        x0_scissored, x1_scissored, y0_scissored, y1_scissored, x0, x1, y0, y1,
        scissor.offset[0], scissor_right, scissor.offset[1], scissor_bottom,
        surface_pitch);
    return true;
  }
  ...
  if (is_depth) {
    ...
    xenos::TextureFormat original_dest_format =
        xenos::TextureFormat(rb_copy_dest_info.copy_dest_format);
    dest_format = GetBaseFormat(original_dest_format);
    if (dest_format != original_dest_format) {
      REXGPU_WARN(
          "Resolving to format {}, normalizing to {} for copy/load compatibility",
          FormatInfo::Get(original_dest_format)->name, FormatInfo::Get(dest_format)->name);
    }
  }
