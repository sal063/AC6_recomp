...
bool IsAc6MovieWorkerWait(uint32_t count, ppc_pu32_t objects_ptr) {
  if (count != 2 || !objects_ptr) {
    return false;
  }
  return objects_ptr[0] == kAc6MovieWorkerWaitEvent0 &&
         objects_ptr[1] == kAc6MovieWorkerWaitEvent1;
}
...
uint32_t xeKeSetEvent(X_KEVENT* event_ptr, uint32_t increment, uint32_t wait) {
  if (uint32_t(reinterpret_cast<uintptr_t>(event_ptr)) == kAc6MovieWorkerSetEvent) {
    REXKRNL_DEBUG("[KeSetEvent] AC6 movie worker event ptr={:08X} increment={} wait={}",
                  uint32_t(reinterpret_cast<uintptr_t>(event_ptr)), increment, wait);
  }
  auto ev = XObject::GetNativeObject<XEvent>(REX_KERNEL_STATE(), event_ptr);
  if (!ev) {
    REXKRNL_WARN("[KeSetEvent] event pointer {:016X} missing native XEvent",
                 uint64_t(reinterpret_cast<uintptr_t>(event_ptr)));
    return 0;
  }
...
ppc_u32_result_t KeWaitForMultipleObjects_entry(ppc_u32_t count, ppc_pu32_t objects_ptr,
  ...
  const bool is_ac6_movie_worker_wait = IsAc6MovieWorkerWait(count, objects_ptr);
  ...
  if (is_ac6_movie_worker_wait) {
    const uint32_t object0 = static_cast<uint32_t>(objects_ptr[0]);
    const uint32_t object1 = static_cast<uint32_t>(objects_ptr[1]);
    REXKRNL_DEBUG(
        "[KeWaitForMultipleObjects] AC6 movie worker wait result={} objects=[{:08X} {:08X}] "
        "wait_type={} reason={} mode={} alertable={} timeout={:016X}",
        static_cast<uint32_t>(result), object0, object1,
        static_cast<uint32_t>(wait_type), static_cast<uint32_t>(wait_reason),
        static_cast<uint32_t>(processor_mode), static_cast<uint32_t>(alertable), timeout);
  }
  if (alertable && result == X_STATUS_USER_APC) {
    XThread::GetCurrentThread()->DeliverAPCs();
  }
