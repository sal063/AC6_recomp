...
ID3D12RootSignature* CreateRootSignature(const D3D12Provider& provider,
  ...
  if (FAILED(provider.SerializeRootSignature(&desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob,
                                             &error_blob))) {
    REXLOG_ERROR("Failed to serialize a root signature");
    if (error_blob != nullptr) {
      REXLOG_ERROR("{}", reinterpret_cast<const char*>(error_blob->GetBufferPointer()));
      error_blob->Release();
    }
    return nullptr;
  }
...
void CreateBufferRawSRV(ID3D12Device* device, D3D12_CPU_DESCRIPTOR_HANDLE handle,
                        ID3D12Resource* buffer, uint32_t size, uint64_t offset) {
