...
## Latest Native Cutscene Sync Pass

The symptom flipped after the native WASAPI cutover:

- game speed/presentation is now correct again
- cutscene audio is no longer late
- cutscene audio is now ahead of visuals

Two concrete changes were made for this pass:

- removed the old movie-worker fast-forward workaround in `src/ac6_audio_hooks.cpp`
  - that workaround was added during the delayed-audio phase
  - it repeatedly re-ran `823B9F10` during preroll to force earlier publish
  - with the newer native path, it was no longer the right behavior and could pull movie audio too early
- added movie-only startup silence injection in the native render path
  - new policy knob: `ac6_movie_audio_startup_silence_frames` default `2`
  - on movie client register, that many silence frames are armed for the first real submit
  - `rex_sub_823A6878` now consumes that budget and injects silence via `XAudioSubmitRenderDriverFrame(driver, 0)` before the first real movie frame
  - kernel/runtime/drivers now treat `samples_ptr == 0` as an intentional silence frame instead of a malformed frame
...
- gameplay audio unchanged
- cutscene movie audio starts slightly later
- first real movie PCM is held behind two native silence frames by default

If cutscene audio is still ahead after this pass, the next safe adjustment is:

- raise `ac6_movie_audio_startup_silence_frames`

If cutscene audio swings back to being late, the next safe adjustment is:

- lower `ac6_movie_audio_startup_silence_frames`
...
User requested a fix attempt without further trace rounds.

Backup created:

- [backup/audio_fix_20260406_182007](/C:/AC6Recomp_ext/backup/audio_fix_20260406_182007)

What was changed:

- [ac6_audio_hooks.cpp](/C:/AC6Recomp_ext/src/ac6_audio_hooks.cpp)
  - `rex_sub_823B7C80` deadline workaround remains in place
- [ac6recomp.toml](/C:/AC6Recomp_ext/out/build/win-amd64-relwithdebinfo/ac6recomp.toml)
  - `ac6_audio_deep_trace = false`
  - `ac6_movie_audio_trace = false`

Workaround rationale:

- the trace narrowed the delayed publish to the `823B7C80 -> 823B9F10` path
- `823B7C80` itself is only a shell
- the producer only wakes once the singleton primary publish advances far enough
- instead of pointer-forcing a specific block, the fix gives the confirmed mutator one extra bounded
  pass while:
  - movie audio is active
  - the render path is still seeing zero-head movie PCM
  - movie submit count is still in the initial preroll window

Why this approach:

- it uses the real guest publish path
- it avoids hardcoding observed block addresses like `207F0CCC` or `207F304C`
- it limits scope to the early cutscene delay window instead of changing the steady-state path

Current behavior of the workaround:

- up to thirty-two extra `823B9F10` passes per `823B7C80` call
- only while `submit_count <= 960`
- only while the producer is still in the zero-head state

This is intentionally a pragmatic deadline fix, not a final root-cause cleanup.

Regression note:

- a later attempt to add supplemental synchronous worker pulses in `rex_sub_823AD9C0` caused
  complete audio loss and has been reverted
...
## Native Host Backend Pivot

Date: 2026-04-06

User requested a deeper repo-wide audio scan and a move away from the SDL-first host path toward a
fully native Windows output path.

Backup state:

- existing deadline-workaround backup remains at
  [backup/audio_fix_20260406_182007](/C:/AC6Recomp_ext/backup/audio_fix_20260406_182007)
- full workspace snapshot created at
  [backup/workspace_20260406_185432](/C:/AC6Recomp_ext/backup/workspace_20260406_185432)

Wide scan findings:

- the current live render-driver path is still host-driver owned and SDL-centered
- `AudioRuntime` still hardcoded `SDLAudioDriver` creation in `RegisterClient` / `Restore`
- `AudioRuntime::backend_name()` still returned the literal `"sdl"`
- `IHostAudioBackend` exists but is not the active playback seam yet
- the rewrite plan already called for:
  - WASAPI shared mode as primary
  - SDL as optional fallback only

Host-side migration started:

- [audio_driver.h](/C:/AC6Recomp_ext/thirdparty/rexglue-sdk/include/rex/audio/audio_driver.h)
  - added generic driver lifecycle hooks:
    - `Initialize()`
    - `Shutdown()`
    - `backend_name()`
- [sdl_audio_driver.h](/C:/AC6Recomp_ext/thirdparty/rexglue-sdk/include/rex/audio/sdl/sdl_audio_driver.h)
  - updated to match the generic driver lifecycle interface
- [wasapi_audio_driver.h](/C:/AC6Recomp_ext/thirdparty/rexglue-sdk/include/rex/audio/wasapi/wasapi_audio_driver.h)
  - new native Windows driver declaration
- [wasapi_audio_driver.cpp](/C:/AC6Recomp_ext/thirdparty/rexglue-sdk/src/audio/wasapi/wasapi_audio_driver.cpp)
  - new event-driven shared-mode WASAPI render driver
  - requests stereo float `48 kHz`
  - tries `IAudioClient3` low-period shared initialization first
  - falls back to classic `IAudioClient::Initialize`
  - uses `AudioCategory_GameMedia` when `IAudioClient2` is available
  - keeps render-driver consumption/accounting connected back into `AudioRuntime`
- [audio_runtime.cpp](/C:/AC6Recomp_ext/thirdparty/rexglue-sdk/src/audio/audio_runtime.cpp)
  - added `audio_backend` CVAR
  - defaults to `wasapi` on Windows
  - creates drivers through a backend selection helper instead of hardcoding SDL
  - falls back to SDL if native WASAPI initialization fails
  - reports the actual selected backend instead of returning the literal `"sdl"`
- [src/audio/CMakeLists.txt](/C:/AC6Recomp_ext/thirdparty/rexglue-sdk/src/audio/CMakeLists.txt)
  - now builds the new WASAPI driver
  - links Windows audio dependencies: `ole32`, `avrt`, `uuid`
- [ac6recomp.toml](/C:/AC6Recomp_ext/out/build/win-amd64-relwithdebinfo/ac6recomp.toml)
  - explicitly sets `audio_backend = "wasapi"`

Important framing:

- this does **not** remove guest-visible `XAudio*` / `XMA*` contracts yet
- it replaces the primary **host sink** first
- that is the safe first cut toward “fully native” without breaking the title’s guest contract

Status after this pivot:

- repo-wide audio inventory: done
- official backend direction check: done enough to justify WASAPI-first implementation
- native host sink wiring: started
- full runtime-owned host backend migration via `IHostAudioBackend`: still pending

Build note:

- no build was run from this session after the WASAPI changes
- user still needs to build locally and report compile/runtime issues from the new backend path

Latest runtime result after local build/test:

- audio output works on the native WASAPI path
- user reports the game is now "really sped up"
- latest trace confirms:
  - backend `wasapi`
...
- the host sink is alive
- but the runtime/tic/callback layer above it is still carrying emulator-shaped cadence assumptions
- host replacement alone is not enough yet

Documentation added:

- [AUDIO_NATIVE_MIGRATION_FILES.md](/C:/AC6Recomp_ext/AUDIO_NATIVE_MIGRATION_FILES.md)
  - file-by-file map from current mixed emulator/native state to the target native recomp design
  - lists what to preserve, what to replace, and the intended replacement files/instructions

Latest fix pass after the speedup report:

- [audio_runtime.cpp](/C:/AC6Recomp_ext/thirdparty/rexglue-sdk/src/audio/audio_runtime.cpp)
  - callback lead is now backend-aware instead of assuming a fixed `2`-frame sink for every host
    driver
  - effective target/low-water now consult each driver's actual device buffer requirements
  - `ConsumeQueuedFramesForClient` no longer advances the audio clock by whole frames on its own;
    it now only retires queue metadata
- [audio_driver.h](/C:/AC6Recomp_ext/thirdparty/rexglue-sdk/include/rex/audio/audio_driver.h)
  - added generic queue target / low-water hints so host drivers can expose their real buffer needs
- [sdl_audio_driver.cpp](/C:/AC6Recomp_ext/thirdparty/rexglue-sdk/src/audio/sdl/sdl_audio_driver.cpp)
  - now reports partial sample consumption to the runtime continuously
  - now reports queue target derived from the actual SDL device buffer size
- [wasapi_audio_driver.cpp](/C:/AC6Recomp_ext/thirdparty/rexglue-sdk/src/audio/wasapi/wasapi_audio_driver.cpp)
  - now reports partial sample consumption to the runtime continuously
  - now exposes queue target derived from the actual WASAPI endpoint buffer size

Intent of this pass:

- stop underfeeding host buffers larger than `2 * 256`
- stop letting tic/accounting lag behind real host writes
- reduce the native path inheriting the old SDL-sized queue assumption

Speedup root cause found after that pass:

- the game speedup was not proven by the steady-state WASAPI cadence itself
- startup was still forcing `ac6_unlock_fps=true` in
  [ac6recomp_app.h](/C:/AC6Recomp_ext/src/ac6recomp_app.h)
- the live trace confirmed this mismatch:
  - config had `ac6_unlock_fps = false`
  - startup log still reported `AC6 audio policy: unlock_fps=true`

Fix applied:

- removed the forced `REXCVAR_SET(ac6_unlock_fps, true);` startup override from
  [ac6recomp_app.h](/C:/AC6Recomp_ext/src/ac6recomp_app.h)

Expected effect:

- AC6 should now honor the config value from `ac6recomp.toml`
- the gross game-speedup symptom should stop if that override was the driver

Follow-up after retest:

- speedup still reproduced even with `unlock_fps=false` confirmed in the startup log
- that ruled out the old forced `ac6_unlock_fps=true` override as the only pacing problem

Additional startup policy issue found:

- [ac6recomp_app.h](/C:/AC6Recomp_ext/src/ac6recomp_app.h) was still forcing:
  - `vsync = false`
  - `guest_vblank_sync_to_refresh = false`
- those were being applied unconditionally, even when `ac6_unlock_fps` was false

Additional fix applied:

- `vsync` and `guest_vblank_sync_to_refresh` are now only forced off when
  `ac6_unlock_fps` is actually enabled
- startup now logs:
  - `AC6 presentation policy: vsync=... guest_vblank_sync_to_refresh=...`

Expected effect of this pass:

- when `ac6_unlock_fps=false`, startup should stop globally forcing unsynced presentation
- the next trace should make the live presentation policy explicit instead of inferred

Cutscene-audio-ahead follow-up:

- after game speed was fixed, user reported cutscene audio now starts ahead of visuals
- steady-state WASAPI path remained stable, so the next likely issue was too much runtime queue
  lead during movie audio

Fix applied:

- [audio_runtime.cpp](/C:/AC6Recomp_ext/thirdparty/rexglue-sdk/src/audio/audio_runtime.cpp)
  - added hot runtime overrides:
    - `audio_force_queue_target_queue_depth`
    - `audio_force_queue_low_water_frames`
- [ac6_audio_policy.cpp](/C:/AC6Recomp_ext/src/ac6_audio_policy.cpp)
  - added AC6 movie-audio queue-lead policy:
    - `ac6_movie_audio_runtime_queue_override`
    - `ac6_movie_audio_queue_target_frames`
    - `ac6_movie_audio_queue_low_water_frames`
  - when movie audio becomes active, policy now clamps runtime queue lead to a tighter target
  - when movie audio ends, the runtime override is cleared

Intent:

- keep normal gameplay audio on the larger backend-aware host lead
- reduce lead only while cutscene/movie audio is active
- address “audio ahead of cutscenes” without reopening the earlier gameplay timing regressions
