...
class XmaContext {
 public:
  ...
  uint32_t guest_ptr() const { return guest_ptr_; }
  bool is_allocated() const { return is_allocated_.load(std::memory_order_acquire); }
  ...
  rex::thread::WaitResult WaitForWorkDone(std::chrono::milliseconds timeout) {
    if (work_completion_event_) {
      return rex::thread::Wait(work_completion_event_.get(), false, timeout);
    }
    return rex::thread::WaitResult::kFailed;
  }
  bool IsIdle();
 ...
 private:
  ...
  size_t PreparePacket(uint32_t frame_size, uint32_t frame_padding);
  ...
  std::unique_ptr<xma::XmaDecoderBackend> decoder_backend_;
