...
#include <rex/math.h>
#include <native/ui/d3d12/d3d12_upload_buffer_pool.h>
...
uint8_t* D3D12UploadBufferPool::Request(uint64_t submission_index, size_t size, size_t alignment,
                                        ID3D12Resource** buffer_out, size_t* offset_out,
                                        D3D12_GPU_VIRTUAL_ADDRESS* gpu_address_out) {
  size_t offset;
  const D3D12Page* page = static_cast<const D3D12Page*>(
...
uint8_t* D3D12UploadBufferPool::RequestPartial(uint64_t submission_index, size_t size,
                                               size_t alignment, ID3D12Resource** buffer_out,
                                               size_t* offset_out, size_t* size_out,
                                               D3D12_GPU_VIRTUAL_ADDRESS* gpu_address_out) {
  size_t offset, size_obtained;
  const D3D12Page* page = static_cast<const D3D12Page*>(GraphicsUploadBufferPool::RequestPartial(
...
GraphicsUploadBufferPool::Page* D3D12UploadBufferPool::CreatePageImplementation() {
  D3D12_RESOURCE_DESC buffer_desc;
  util::FillBufferResourceDesc(buffer_desc, page_size_, D3D12_RESOURCE_FLAG_NONE);
  Microsoft::WRL::ComPtr<ID3D12Resource> buffer;
  if (FAILED(provider_.GetDevice()->CreateCommittedResource(
          &util::kHeapPropertiesUpload, provider_.GetHeapFlagCreateNotZeroed(), &buffer_desc,
          D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&buffer)))) {
    REXLOG_ERROR("Failed to create a D3D upload buffer with {} bytes", page_size_);
    return nullptr;
  }
  ...
  if (FAILED(buffer->Map(0, &read_range, &mapping))) {
    REXLOG_ERROR("Failed to map a D3D upload buffer with {} bytes", page_size_);
    buffer->Release();
    return nullptr;
  }
...
D3D12UploadBufferPool::D3D12Page::D3D12Page(ID3D12Resource* buffer, void* mapping)
