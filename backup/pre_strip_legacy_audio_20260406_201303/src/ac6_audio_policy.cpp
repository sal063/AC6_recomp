...
REXCVAR_DEFINE_BOOL(ac6_movie_audio_runtime_queue_override, true, "AC6",
                    "Clamp runtime queue lead while AC6 movie audio is active");
REXCVAR_DEFINE_INT32(ac6_movie_audio_queue_target_frames, 2, "AC6",
                     "Runtime queue target to use while AC6 movie audio is active").range(1, 8);
REXCVAR_DEFINE_INT32(ac6_movie_audio_queue_low_water_frames, 1, "AC6",
                     "Runtime low-water threshold to use while AC6 movie audio is active").range(0, 7);
REXCVAR_DEFINE_INT32(ac6_movie_audio_startup_silence_frames, 2, "AC6",
                     "Extra silence frames to inject before the first AC6 movie-audio submit")
    .range(0, 8);

REXCVAR_DECLARE(bool, ac6_timing_hooks_enabled);
REXCVAR_DECLARE(int32_t, audio_force_queue_low_water_frames);
REXCVAR_DECLARE(int32_t, audio_force_queue_target_queue_depth);
...
struct MovieAudioState {
    ...
    uint32_t startup_silence_frames_remaining{0};
...
bool g_movie_audio_queue_override_active{false};
...
void UpdateRuntimeQueueOverrideLocked() {
    const bool should_override =
        REXCVAR_GET(ac6_movie_audio_runtime_queue_override) &&
        ComputeMovieAudioActiveLocked();
    if (should_override == g_movie_audio_queue_override_active) {
        return;
    }

    g_movie_audio_queue_override_active = should_override;
    if (should_override) {
        REXCVAR_SET(audio_force_queue_target_queue_depth,
                    REXCVAR_GET(ac6_movie_audio_queue_target_frames));
        REXCVAR_SET(audio_force_queue_low_water_frames,
                    REXCVAR_GET(ac6_movie_audio_queue_low_water_frames));
    } else {
        REXCVAR_SET(audio_force_queue_target_queue_depth, 0);
        REXCVAR_SET(audio_force_queue_low_water_frames, -1);
    }

    if (REXCVAR_GET(ac6_movie_audio_trace) || ac6::audio_policy::IsDeepTraceEnabled()) {
        REXAPU_DEBUG(
            "AC6 movie-audio runtime queue override active={} target={} low_water={}",
            should_override,
            REXCVAR_GET(audio_force_queue_target_queue_depth),
            REXCVAR_GET(audio_force_queue_low_water_frames));
    }
}
...
bool IsMovieAudioActive() {
    ...
    UpdateRuntimeQueueOverrideLocked();
...
void OnMovieAudioClientRegistered(const uint32_t owner_ptr,
    ...
    client.startup_silence_frames_remaining =
...
uint32_t ConsumeMovieAudioStartupSilenceFrames(const uint32_t owner_ptr,
    ...
    if (!client || client->submit_count != 0 ||
        client->startup_silence_frames_remaining == 0) {
        return 0;
    }

    const uint32_t silence_frames = client->startup_silence_frames_remaining;
    client->startup_silence_frames_remaining = 0;
    if (REXCVAR_GET(ac6_movie_audio_trace) || IsDeepTraceEnabled()) {
        REXAPU_DEBUG(
            "AC6 movie-audio startup silence owner={:08X} driver={:08X} frames={}",
            owner_ptr,
            driver_ptr,
            silence_frames);
    }
