...
ID3D12Resource* D3D12CommandProcessor::RequestScratchGPUBuffer(uint32_t size,
  ...
  if (!submission_open_) {
    REXGPU_WARN("RequestScratchGPUBuffer called with no open submission");
    return nullptr;
  }
  if (scratch_buffer_used_) {
    REXGPU_WARN("RequestScratchGPUBuffer called while scratch buffer is already in use");
    return nullptr;
  }
  if (size == 0) {
    return nullptr;
...
bool D3D12CommandProcessor::PresentDirectSwap(
    const system::GraphicsSwapSubmission& submission) {
  const uint32_t target_write_ptr = write_ptr_index_.load(std::memory_order_acquire);
  if (system::XThread::IsInThread(worker_thread_.get())) {
    return PresentDirectSwapOnWorkerThread(submission, target_write_ptr);
  }

  thread::Fence fence;
  bool result = false;
  CallInThread([this, submission, target_write_ptr, &result, &fence]() {
    result = PresentDirectSwapOnWorkerThread(submission, target_write_ptr);
    fence.Signal();
  });
...
bool D3D12CommandProcessor::PresentDirectSwapOnWorkerThread(
    const system::GraphicsSwapSubmission& submission, uint32_t target_write_ptr) {
  if (target_write_ptr != 0xBAADF00D && read_ptr_index_ != target_write_ptr) {
    read_ptr_index_ = ExecutePrimaryBuffer(read_ptr_index_, target_write_ptr);
    if (read_ptr_writeback_ptr_) {
      memory::store_and_swap<uint32_t>(memory_->TranslatePhysical(read_ptr_writeback_ptr_),
                                       read_ptr_index_);
    }
    if (read_ptr_index_ != target_write_ptr) {
      REXGPU_ERROR(
          "PresentDirectSwap: command processor failed to drain to the VdSwap "
          "write pointer {:08X} (reached {:08X})",
          target_write_ptr, read_ptr_index_);
      return false;
    }
  }

  for (uint32_t i = 0; i < submission.texture_fetch.size(); ++i) {
    WriteRegister(XE_GPU_REG_SHADER_CONSTANT_FETCH_00_0 + i, submission.texture_fetch[i]);
  }

  rex::debug::Profiler::Flip();
  return IssueSwapInternal(submission.frontbuffer_physical_address,
                           submission.frontbuffer_width,
...
void D3D12CommandProcessor::IssueSwap(uint32_t frontbuffer_ptr, uint32_t frontbuffer_width,
  ...
  (void)IssueSwapInternal(frontbuffer_ptr, frontbuffer_width, frontbuffer_height);
}

bool D3D12CommandProcessor::IssueSwapInternal(uint32_t frontbuffer_ptr,
       ...
       guest_output_height](ui::Presenter::GuestOutputRefreshContext& context) -> bool {
        ...
        if (!(use_pwl_gamma_ramp ? gamma_ramp_pwl_up_to_date_
          ...
          if (std::endian::native != std::endian::little && use_pwl_gamma_ramp) {
            ...
            if (swap_texture_resource == direct_display_texture_.Get() && !use_pwl_gamma_ramp) {
              reg::DC_LUT_30_COLOR identity_ramp[256];
              for (uint32_t i = 0; i < 256; ++i) {
                uint32_t val10 = (i << 2) | (i >> 6);
                identity_ramp[i].value = 0;
                identity_ramp[i].color_10_red = val10;
                identity_ramp[i].color_10_green = val10;
                identity_ramp[i].color_10_blue = val10;
                identity_ramp[i]._pad_30 = 3; // Alpha = 1.0
              }
        ...
        if (!RequestOneUseSingleViewDescriptors(bindless_resources_used_ ? 2 : 3,
          ...
          REXGPU_ERROR("RefreshGuestOutput: RequestOneUseSingleViewDescriptors failed");
          return false;
        }
...
bool D3D12CommandProcessor::EndSubmission(bool is_swap) {
  ...
  if (submission_open_) {
    if (scratch_buffer_used_) {
      REXGPU_ERROR("EndSubmission: scratch_buffer_used_ is true at submission close! Clearing to avoid crash.");
      scratch_buffer_used_ = false;
    }
...
ID3D12Resource* rex::graphics::d3d12::D3D12CommandProcessor::UpdateDirectDisplayTexture(
    uint32_t frontbuffer_ptr, uint32_t frontbuffer_width, uint32_t frontbuffer_height,
    D3D12_SHADER_RESOURCE_VIEW_DESC& srv_desc_out, rex::graphics::xenos::TextureFormat& format_out,
    uint32_t* width_unscaled_out, uint32_t* height_unscaled_out) {
  ...
  if ((frontbuffer_ptr & 0xFFF) == 0 &&
      frontbuffer_ptr >= 0x1000 && frontbuffer_ptr < 0x20000000 &&
      frontbuffer_ptr != 0xFEFEFEFE && frontbuffer_ptr != 0xDEADBEEF &&
      frontbuffer_ptr != 0xCDCDCDCD && frontbuffer_ptr != 0xBAADF00D) {
    is_direct_fb = true;
    fb_addr = frontbuffer_ptr;
  } else {
    uint8_t* ptr = memory_->TranslatePhysical(frontbuffer_ptr & 0x1FFFFFFF);
    if (!ptr) {
      REXGPU_ERROR("UpdateDirectDisplayTexture: pointer translation failed");
      return nullptr;
    }
    const uint32_t* swap_struct = reinterpret_cast<const uint32_t*>(ptr);
    static uint32_t s_cached_fb_addr = 0;
    
    auto IsValidFbAddr = [](uint32_t v) -> bool {
      return (v & 0xFFF) == 0 &&
             v >= 0x1000 && v < 0x20000000 &&
             v != 0xFEFEFEFE && v != 0xDEADBEEF &&
             v != 0xCDCDCDCD && v != 0xBAADF00D;
    };

    for (int i = 0; i < 8 && fb_addr == 0; ++i) {
      uint32_t candidate = rex::byte_swap(swap_struct[i]);
      if (IsValidFbAddr(candidate)) {
        fb_addr = candidate;
      }
    }

    if (fb_addr == 0) {
      if (s_cached_fb_addr != 0) {
        fb_addr = s_cached_fb_addr;
      } else {
        REXGPU_ERROR("UpdateDirectDisplayTexture: structured search failed");
        return nullptr;
      }
    } else {
      s_cached_fb_addr = fb_addr;
    }
  }

  uint8_t* fb_data = memory_->TranslatePhysical(fb_addr);
  if (!fb_data) {
    REXGPU_ERROR("UpdateDirectDisplayTexture: fb_data mapping failed");
    return nullptr;
  }
  ...
  if (!direct_display_texture_) {
    ...
    if (FAILED(device->CreateCommittedResource(
            ...
            D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&direct_display_texture_)))) {
      REXGPU_ERROR("UpdateDirectDisplayTexture: CreateCommittedResource failed");
      return nullptr;
    }
    direct_display_texture_state_ = D3D12_RESOURCE_STATE_COMMON;
  }

  if (direct_display_texture_state_ != D3D12_RESOURCE_STATE_COPY_DEST) {
    PushTransitionBarrier(direct_display_texture_.Get(), direct_display_texture_state_, D3D12_RESOURCE_STATE_COPY_DEST);
    direct_display_texture_state_ = D3D12_RESOURCE_STATE_COPY_DEST;
    SubmitBarriers();
  }
  ...
  for (uint32_t y = 0; y < height; y += chunk_height) {
    ...
    uint32_t request_size = rows_to_copy * row_pitch;

    uint64_t upload_offset = 0;
    ID3D12Resource* upload_buffer_resource = nullptr;
    D3D12_GPU_VIRTUAL_ADDRESS upload_gpu_addr = 0;
    uint8_t* upload_mapping = constant_buffer_pool_->Request(
        submission_current_, request_size, D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT,
        &upload_buffer_resource, (size_t*)&upload_offset, &upload_gpu_addr);

    if (upload_mapping) {
      for (uint32_t cy = 0; cy < rows_to_copy; ++cy) {
        if (fb_addr + (y + cy) * width * bpp + width * bpp <= 0x20000000) {
          uint8_t* src_ptr = fb_data + (y + cy) * width * bpp;
      ...
      dst_loc.pResource = direct_display_texture_.Get();
      dst_loc.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX;
      dst_loc.SubresourceIndex = 0;

      src_loc.pResource = upload_buffer_resource;
      src_loc.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
      src_loc.PlacedFootprint.Offset = upload_offset;
      src_loc.PlacedFootprint.Footprint.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
      src_loc.PlacedFootprint.Footprint.Width = width;
      src_loc.PlacedFootprint.Footprint.Height = rows_to_copy;
      src_loc.PlacedFootprint.Footprint.Depth = 1;
      src_loc.PlacedFootprint.Footprint.RowPitch = row_pitch;

      deferred_command_list_.D3DCopyTextureRegion(&dst_loc, 0, y, 0, &src_loc, nullptr);
    } else {
      REXGPU_ERROR("UpdateDirectDisplayTexture: Pool upload_mapping failed for y={}", y);
      break;
    }
  }
  ...
  format_out = rex::graphics::xenos::TextureFormat::k_8_8_8_8;
  if (width_unscaled_out) *width_unscaled_out = width;
  if (height_unscaled_out) *height_unscaled_out = height;

  return direct_display_texture_.Get();
}
