...
class D3D12CommandProcessor : public CommandProcessor {
 public:
  ...
  bool PresentDirectSwap(const system::GraphicsSwapSubmission& submission);
 ...
 private:
  ...
  static constexpr uint64_t kMemexportReadbackBufferEvictionAgeFrames = 30 * 60;
  ...
  static inline uint64_t MakeMemexportReadbackKey(
      const std::vector<draw_util::MemExportRange>& ranges, uint32_t total_size) {
    uint64_t key = 1469598103934665603ull;
    key ^= uint64_t(ranges.size());
    key *= 1099511628211ull;
    key ^= uint64_t(total_size);
    key *= 1099511628211ull;
    for (const draw_util::MemExportRange& range : ranges) {
      key ^= (uint64_t(range.base_address_dwords) << 32) | uint64_t(range.size_bytes);
      key *= 1099511628211ull;
    }
    return key;
  }
  ...
  Microsoft::WRL::ComPtr<ID3D12Resource> direct_display_texture_;
  D3D12_RESOURCE_STATES direct_display_texture_state_ = D3D12_RESOURCE_STATE_COMMON;

  bool IssueSwapInternal(uint32_t frontbuffer_ptr, uint32_t frontbuffer_width,
                         uint32_t frontbuffer_height);
  bool PresentDirectSwapOnWorkerThread(const system::GraphicsSwapSubmission& submission,
                                       uint32_t target_write_ptr);
  ID3D12Resource* UpdateDirectDisplayTexture(
